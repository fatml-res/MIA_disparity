...
if __name__ == "__main__":
    ...
    files = find_all_results("Fairpick")
    replace = 0
    for file in files:
        folder = file.split("/target_result.csv")[0]
        if "Adult" not in file or float(file.split('/')[1].replace('n=',''))<0:
            pass
            print("Skip {} because we are not working on this dataset/setting".format(file))
            continue
        if os.path.exists(folder + "/attack_result_2.csv") and replace == 0:
            print("Skipped " + file)
            continue
        else:
        ...
        for c in [0, 1]:
            ...
            group_c_train = group_train[inds]
            All_data_train = np.hstack((group_c_train,
                                    y_c.reshape(-1, 1),
                                    x_prob,
                                    s_c.reshape(-1, 1),
                                    X_train[inds, 0].reshape(-1, 1)))
            tmp_all.append(All_data_train)
            # testing
        ...
        final_df.to_csv(folder + "/attack_result_2.csv", header=False, index=False)
