...
def run_l2(dataset, lbd=0.001, time=0, num_epoch=455):
    torch.set_num_threads(15)
    torch.set_num_interop_threads(15)
    target_file = "L2/l2={}/influence_{}_{}.npy".format(lbd, dataset, time)
    if os.path.exists(target_file):
        print("Skip {} because it is done".format(target_file))
        return 0
    if not os.path.exists("L2/l2={}".format(lbd)):
        os.mkdir("L2/l2={}".format(lbd))
...
def l2_gradient(dataset, lbd=0.001, time=0, num_epoch=455):
    torch.set_num_threads(15)
    #torch.set_num_interop_threads(15)
    target_file = "L2/l2={}/acc_{}_{}.npy".format(lbd, dataset, time)
    if os.path.exists(target_file):
        pass
        #print("Skip {} because it is done".format(target_file))
        #return 0
...
def l2_target(dataset, lbd=0.001, time=0, num_epoch=455):
    torch.set_num_threads(15)
    data1 = pd.read_csv("One_hot/oh{}.csv".format(dataset)).to_numpy()
    X_train, X_test, y_train, y_test = train_test_split(data1[:, 0:-1], data1[:, -1], test_size=0.5)
    ...
    for epoch in range(0, num_epoch):
        ...
        if epoch + 1 in [20, 40, 80, 160, 320, 400]:
            target_file = "gradients/L2/l2={}/{}/epoch={}/target_result.csv".format(lbd, dataset, epoch+1)
            if os.path.exists(target_file):
                print("Skip {} because it is done".format(target_file))
                continue
            if not os.path.exists("gradients/L2/l2={}/{}/epoch={}".format(lbd, dataset, epoch+1)):
                os.makedirs("gradients/L2/l2={}/{}/epoch={}".format(lbd, dataset, epoch+1))
        ...
        scheduler.step()
    norm_list = np.vstack(norm_list)
    target_file = "gradients/L2/l2={}/gradient_{}.npy".format(lbd, dataset)
    np.save(target_file, norm_list)
...
if __name__ == "__main__":
    for time in range(1):
        for i in range(1):
            ...
            num_epoch = [400, 400, 300][i]
            for l2 in [1e-4, 1e-3, 1e-2, 0.1]:
                #run_l2(dataset, l2, time, round(num_epoch, 2))\
                l2_target(dataset, l2, time, num_epoch)