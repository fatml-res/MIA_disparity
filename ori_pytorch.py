...
def run_ori(dataset, time=0, num_epoch=455):
    target_file = "Original/subscore/influence_{}_{}.npy".format(dataset, time)
    if os.path.exists(target_file):
        print("Skip {} because it is done".format(target_file))
        return 0
    if not os.path.exists("Original/subscore"):
        os.mkdir("Original/subscore/")
...
def ori_target(dataset, time=0, num_epoch=455):
    ...
    for epoch in range(0, num_epoch):
        ...
        if epoch + 1 in [20, 40, 80, 160, 320, 400]:
            target_file = "gradients/ori/{}/epoch={}/target_result.csv".format(dataset, epoch + 1)
            if os.path.exists(target_file):
                print("Skip {} because it is done".format(target_file))
                continue
            if not os.path.exists("gradients/ori/{}/epoch={}".format(dataset, epoch + 1)):
                os.makedirs("gradients/ori/{}/epoch={}".format(dataset, epoch + 1))
    ...
    target_file = "gradients/ori/gradient_{}.npy".format(dataset)
...
if __name__ == "__main__":
    for time in range(1):
        for i in range(1):
            ...
            ori_target(dataset, time, num_epoch)